...
// Update the "creq->access_type" field of an outstanding writeback/invalidate
// message with the result of a cache_coher_yield() call.  See
// enq_coher_peer_msgs() for cheesily overloaded meaning of the
// "creq->access_type" field.
static void
...
process_l2access(CacheRequest *creq)
{
    ...
    ready_time = 
        cache_update_bank(l2cache, creq->base_addr, cyc,
    ...
    if (cache_stat == Cache_Hit) {
	  ...
	  DEBUGPRINTF("cache: promoting C%d's access to addr %s to "
        ...
        if (private_l2) {
            ...
            creq->request_time = ready_time +
                first_core->
                params.private_l2cache.timing.miss_penalty;
            creq->action = BUS_REQ;
        } else {
...
process_l3access(CacheRequest *creq)
{
    ...
    ready_time = 
        cache_update_bank(l3cache, creq->base_addr, cyc,
                          CacheBank_LookupREx);
...
process_l3wb(CacheRequest *creq)
{
 ...
 //   printf("tick %s %s\n",fmt_now(), fmt_laddr(creq->base_addr));
    ...
    if (hit) {
        ...
        // note: L3 writeback buffer allocated
        creq->request_time = ready_time +
            GlobalParams.mem.l3cache_timing.miss_penalty;
        creq->action = MEM_WB;
        place_in_cache_queue(creq);
...
/* this is an event-driven simulator of the cache hierarchy -- events
   are kept in chronological order, so that if there is anything to do
   this cycle, it is at the head of the queue.

   Accesses will come into the queues through doiaccess or dodaccess,
   if there is a miss,
   make several transitions, and then leave through the L1FILL state.

   Duplicate requests will be merged into a single request.  This 
   process is pretty complex and still prone to bugs, 
   some of which I have to admit I
   just tolerate.
*/
...
// returns: zero (CacheMerge_NoMerge) if no relevant outstanding request, some
// other value iff responsibility for this access now lies with some
// CacheRequest (which may or may not be new, but the caller does not need to
// schedule it).  If "pending_req_ret" is non-NULL, a copy of the relevant
// CacheRequest pointer (or NULL) will be written to it.
//
// req_ctx: NULL <=> "naked" request from "req_core" (e.g. prefetch)
// req_inst_or_neg1: >=0 with dcache access from "req_ctx->alist[req_inst...]"
//                   -1 otherwise (icache, streambuf, etc.)
//
// This is complicated, icky, and doesn't directly model any one piece of
// a processor; it models a hodgepodge of load/store queues and MSHRs
                          ...
                          CacheRequest **pending_req_ret)
{
    ...
    if ((merge_type != 0) && l1cache &&
        ...
        // Odd case: the local L1 cache (I or D) already has the desired
        // block, with sufficient permission to satisfy this request, yet
        // there is also an outstanding miss for this address, which involves
        // this core somehow.
        //
        // If the found request does not involve this cache type (I vs. D),
        // we'll ignore it and allow this request to proceed without merging;
        // given its presence in the cache, it will end up as a hit in 
        // do[id]access().
        //
        // This may seem impossible, that an outstanding miss should imply
        // that the data be absent, but split I and D caches which aren't kept
        // coherent with each other can lead to this.  (e.g. D-cache has a
        // block dirty, I-cache has a miss for that block, then a store to
        // that block occurs and is merged with the outstanding I-miss,
        // leading to a D-fill from below on a dirty block, which causes an
        // assertion failure.)
                ...
                CacheAccessType access_type, int *tlb_miss_ret)
{
    ...
    DEBUGPRINTF("cache: time %s addr %s +%d, core %d I-cache access %s: %s, "
    ...
    if (core->params.icache.prefetch_nextblock && is_first_touch) {
        ...
        DEBUGPRINTF("cache: core %d I-cache prefetch_nextblock: %s "
                ...
                int *upgrade_miss_ret, int *streambuf_hit_ret)
{
    ...
    *upgrade_miss_ret = 0;
    ...
    //printf ("cache:   penalty= %d        base_addr=%d        addr_ready_cyc=%d      core_ide=%d\n",penalty,base_addr,addr_ready_cyc,core->core_id);
            ...
            for (int i = 0; pending_req->cores[i].core; ++i) {
                if (pending_req->cores[i].core == core) {
                    if (CACHE_SOURCE_L1_CONTAINS(pending_req->cores[i].src,
                                                 CSrc_L1_DStreamBuf)) {
                        // Warning: this isn't a PF win for sure; consider
                        // sequence D-miss -> PF(merged,PF loses) ->
                        // D-miss(merged)
...
// Report best-case (contention-free) data-access latencies, for hits at
// various levels in the cache hierarchy.  Latencies are measured from the
// start of the cycle an address becomes available to the memory subsystem, to
// the start of the cycle the requested data is available for use.
// (Note that these hit times may be encountered rarely, particularly with
// evict-at-fill in use, as the eviction delays the fill.)
//
// This doesn't actually _do_ anything to the simulation, but it provides an
// easy-to-read output summing the appropriate latency components.  The
// figures reported should exactly match those reported as "total mem_delay"
// by process_dfill_inst().
static void
