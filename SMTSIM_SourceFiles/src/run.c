...
i64 instructions_completed = 0;
i64 present_instructions = 0;
i64 prev_instructions = 0;
i64 cycle_prev=0;
i64 cycle_interval=0;
double ipc_now = 0;
 ...
 */
	...
	if((cyc>0)&&(cyc%10000==0)&&(cyc<=10000000))
		{
		present_instructions=as->extra->total_commits;
		instructions_completed = present_instructions - prev_instructions;
		cycle_interval = cyc - cycle_prev;
		ipc_now = (double)instructions_completed/cycle_interval;
		printf("Instructions :%s		IPC: %f	\n",fmt_i64(instructions_completed),ipc_now);
		cycle_prev = cyc;
		prev_instructions=present_instructions;
		}
	}
        cyc++;
